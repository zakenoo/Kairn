...
There is also one thing Kairn deliberately does *not* do: it never asks you to sort, rate or prioritise. There is a single way to say “this one counts”: a star, one click, three at most.
...
No. Reminders are fired by Kairn itself, so it has to be running. That is what “start with Windows” and the notification area are for. Nothing is scheduled in Windows behind your back.
</details>
